...
final class Runner extends Thread {
    ...
    final void addCustomer(final long customerId) {
        synchronized(_customers) {
            _customers.add(customerId);
        }
    }

    final long getCustomer() {
        synchronized(_customers) {
            if (_customers.size() > 0) {
                return _customers.get(_random.nextInt(_customers.size()));
            }
            return -1;
        }
    }

    final long removeCustomer() {
        synchronized(_customers) {
            if (_customers.size() > 0) {
                return _customers.remove(_random.nextInt(_customers.size()));
            }
            return -1;
        }
    }
